...
const getUserElements = (
  fullname: string
): PromiseLike<IUserElements[] | undefined> => {
...
export function Tab() {
  const { loading, elements } = useGetSummedDebt();
  ...
  const getUserRows = useMemo(() => {
    ...
    const userTotalOrdered = userElements.reduce(
      (acc, val) => acc + val.ordered,
      0
    );
        ...
        <Stack>
          <Group justify="space-between">
            ...
            <Badge
              ...
              radius="sm"
              size="lg"
              color={userTotalOwed <= 0 ? "green" : "red"}
            >
              {numberToEur(userTotalOwed)} €
            </Badge>
            <Group justify="right">
              <Group>
              ...
              <Button variant="outline" onClick={addGajba(userFullName)}>
                Dodaj gajbo
              </Button>
            </Group>
          </Group>
